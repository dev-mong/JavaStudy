...
public class PhoneBookManager {
	...
	private PhoneBookManager(){
		...
		length=0;
		index=-1;
		phoneData=new ArrayList<PhoneBookDataAbstract>();
	...
	int searchName(String nameInput) {
		...
		//리스트로 같은 이름 존재 여부 확인 
		for(int i=0;i<phoneData.size();i++) { //phoneData 배열 반복
			if(phoneData.get(i).name(nameInput)) { //phoneData객체 배열에 이름을 검색 
	...
	void update() {
		...
		if(index<0) {
			...
			if(phoneData.get(index) instanceof PhoneFriendInfor) {
			...
			else if(phoneData.get(index) instanceof PhoneUnivlnfor){
				...
				String year=sc.nextLine();
				data= new PhoneUnivlnfor(name, phoneNumber, address, email, major, year);
			} else if(phoneData.get(index) instanceof PhoneCompanyInfor) {
				System.out.println("3.이메일");
				String email = sc.nextLine();
				System.out.println("4.회사이름");
				String company=sc.nextLine();
				data=new PhoneCompanyInfor(name, phoneNumber, email, company);
			} else if(phoneData.get(index) instanceof PhoneBookClubInfor) {
